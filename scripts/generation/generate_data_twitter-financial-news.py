...
MODEL = "gpt-4o"
INPUT_JSONL = "../outputs/2seed_data_twitter_random.jsonl"
OUTPUT_JSONL = "../outputs/2synthetic_data_from_Seed_random.jsonl"
SLEEP_TIME = 1.2
...
assert {"instruction", "input", "output"}.issubset(df.columns), "JSONL missing required keys"
...
Examples:
1. {examples['Bearish']} → Bearish
2. {examples['Bullish']} → Bullish
3. {examples['Neutral']} → Neutral
...
for _, row in tqdm(df.iterrows(), total=len(df)):
    ...
    try:
        ...
        other_seeds = df[(df['output'] == label) & (df['input'] != sentence)].sample(
            n=2, random_state=row_seed)['input'].tolist()
