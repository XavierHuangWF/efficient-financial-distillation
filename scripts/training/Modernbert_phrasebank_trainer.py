    ...
    if source == "random":
        ...
        for cls in sorted(df["label"].unique()):
            subset = df[df["label"] == cls].sample(
                n=min(samples_per_class, (df["label"] == cls).sum()),
                random_state=seed,
                replace=False,
            )
...
def normalize_label_to_phrasebank_id(lbl: str) -> int:
...
def load_jsonl_to_df(path: str) -> pd.DataFrame:
    ...
    df["sentence"] = df["sentence"].astype(str).apply(clean_text)
    df["label"] = df["label"].apply(normalize_label_to_phrasebank_id).astype(int)
    return df
...
def build_train_val_test(args):
    ...
    train_pool_df, val_df, test_df = stratified_split(full_data, seed=args.seed)
    ...
    if args.mode == "full":
        ...
        if args.seed_source == "file":
            if not args.seed_jsonl:
                raise ValueError("--seed-jsonl is required when --seed-source file")
            file_df = load_jsonl_to_df(args.seed_jsonl)
            merged = file_df.merge(
                train_pool_df[["sentence", "label"]],
                on=["sentence", "label"],
                how="inner",
            )
            ...
            print(f"Loaded seed-file training size: {len(train_df)}")
        else:
        ...
        syn_df = load_jsonl_to_df(args.synthetic_jsonl)
        print(f"Synthetic training size (pre-merge): {len(syn_df)}")

        if args.seed_source == "file":
            if not args.seed_jsonl:
                raise ValueError("--seed-jsonl is required when --seed-source file")
            seed_df = load_jsonl_to_df(args.seed_jsonl)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Unified ModernBERT PhraseBank trainer")

    parser.add_argument("--mode", choices=["full", "seed", "synthetic"], required=True)
    parser.add_argument("--seed-source", choices=["file", "random", "cluster"], default="file")
...
if __name__ == "__main__":
    ...
    freeze_first_n_layers(model, args.freeze_layers)
