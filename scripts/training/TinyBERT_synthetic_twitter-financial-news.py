...
# UPDATED: switch to BERT (prajjwal1/bert-tiny)
...
NUM_EPOCHS = 50
FREEZE_LAYERS = 4
USE_SYNTHETIC_DATA = False   # train on synthetic + seed data (merged)
USE_SEED_ONLY =  True      # train on seed data only
USE_EARLY_STOPPING = True

SYNTHETIC_PATH = "../outputs/synthetic_data_from_Seed.jsonl"
SEED_PATH = "../outputs/seed_data.jsonl"

# ---------- helpers for the zeroshot dataset ----------
TEXT_CANDIDATES = ["sentence", "text", "tweet", "content", "document", "message", "headline"]

# Canonical mapping for this dataset
...
# UPDATED: concat HF train + validation, then do our own split
def DataLoad():
    # Load both provided splits and combine, then keep your stratified split downstream.
    ...
    # Prefer explicit label text if available; fallback to numeric labels (0/1/2)
...
# UPDATED: tokenizer uses BERT-tiny
...
# ===== NEW: load+normalize JSONL and merge synthetic + seed =====
def _normalize_label_to_id(x: str) -> int:
    """Accept 'Bearish/Bullish/Neutral' or 'Negative/Positive/Neutral' -> 0/1/2"""
    if not isinstance(x, str):
        raise ValueError(f"Label is not a string: {x}")
    y = x.strip().title()
    if y == "Negative":
        y = "Bearish"
    elif y == "Positive":
    ...
    if y not in LABEL_ID_BY_NAME:
        raise ValueError(f"Unexpected label value in JSONL: {x}")
...
if __name__ == "__main__":
    ...
    if USE_SYNTHETIC_DATA:
        ...
        if os.path.exists(SEED_PATH):
            ...
            # de-duplicate exact (sentence, label) pairs
            before = len(train_df)
        ...
        # build validation/test from HF dataset
        full_data = DataLoad()
    ...
    # UPDATED: switch to prajjwal1/bert-tiny
    model = BertForSequenceClassification.from_pretrained("prajjwal1/bert-tiny", num_labels=3, use_safetensors=True)

    # UPDATED: safe clamp for BERT-tiny (2 encoder layers total)
    if FREEZE_LAYERS > 0:
        try:
            ...
            for param in model.bert.encoder.layer[:freeze_upto].parameters():
                param.requires_grad = False
        except Exception as e:
            print(f"Warning: could not freeze layers cleanly: {e}")
    else:
    ...
    # UPDATED: report names in the same order as label IDs
