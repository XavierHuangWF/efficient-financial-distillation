...
NUM_EPOCHS = 50
FREEZE_LAYERS = 4
USE_SYNTHETIC_DATA =   False # now: trains on synthetic + seed (merged)
USE_SEED_ONLY   = True
...
# --- helpers to load/normalize JSONL and MERGE syn + seed ---
def _normalize_label_to_id(lbl: str) -> int:
    # Your script evaluates/reporting in order: Negative(0), Neutral(1), Positive(2)
    # Accept both naming schemes:
    #   Negative/Neutral/Positive  -> 0/1/2
    #   Bearish/Neutral/Bullish    -> 0/1/2   (Bearish->Negative, Bullish->Positive)
...
def _load_jsonl_as_df(path: str) -> pd.DataFrame:
    ...
    # Expect instruction/input/output in your JSONLs
...
if __name__ == "__main__":
    ...
    if USE_SYNTHETIC_DATA:
        ...
        full_data = DataLoad()
        # PhraseBank labels are already 0/1/2 for Negative/Neutral/Positive
        full_data = full_data.rename(columns={"sentence": "sentence", "label": "label"})
